...
class SaleOrderLine(models.Model):
    ...
    def _compute_report_subtotal(self):
        ...
        sub = self.env['sale.subscription.line'].search([('analytic_account_id.state','=','open'), ('sub_account_id.parent_id', '=', self.order_id.partner_id.id), ('sub_account_id', '=', self.sub_account_id.id), ('name', '=', self.name), ('product_id', '=', self.product_id.id), ('analytic_account_id.date_start', '<=', self.order_id.create_date)], limit=1)
        for line in self:
